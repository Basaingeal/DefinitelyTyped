...
function passwordBasedSignIn() {
    ...
    navigator.credentials.get({ password: true }).then(credential => {
        ...
        if (credential.type === 'password') {
            ...
            form.append('password_field', (credential as PasswordCredential).password || '');
    ...
    navigator.credentials
        ...
        .then(credential => {
            ...
            if (credential.type === 'federated') {
                const federatedCredential = credential as FederatedCredential;
                switch (federatedCredential.provider) {
                    ...
                    default:
                        fetch('https://example.com/loginEndpoint', { credentials: 'include', method: 'POST' });
                        break;
                }
            } else {
...
function passwordPostSignInConfirmation() {
    ...
    document.querySelector('#theForm')!.addEventListener('submit', e => {
        if (navigator.credentials) {
            ...
            fetch(formElem.action, { method: 'POST', credentials: 'include' }).then(r => {
                if (r.status === 200) {
...
function federationPostSignInConfirmation() {
    if (navigator.credentials) {
        navigator.credentials.store(
            new FederatedCredential({ id: 'username', provider: 'https://federation.com', origin: 'federation.com' })
        );
    }
}
...
function createPasswordCredential() {
    ...
    navigator.credentials
        .create({
            password: { id: 'username', password: 'password', origin: 'localhost' },
        })
        .then(credential => {
            // Credential created!
        });
...
function createFederatedCredential() {
    ...
    navigator.credentials
        .create({
            federated: { id: 'username', provider: 'provider', origin: 'localhost' },
        })
        .then(credential => {
            // Credential created!
        });
